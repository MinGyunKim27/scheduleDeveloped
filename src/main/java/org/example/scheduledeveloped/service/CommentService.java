...
public class CommentService {
     ...
     */
    public CommentResponseDto updateComments(Long id, CommentRequestDto requestDto, SessionUserDto sessionUserDto) {
        Comment comment = commentRepository.findById(id)
                .orElseThrow(() -> new CommentNotFoundException("해당 Comment를 찾을 수 없습니다."));

        validateCommentOwner(comment, sessionUserDto.getId());
     ...
     * @throws AccessDeniedException 사용자가 댓글 작성자가 아닐 경우
     */
    public Long deleteComment(Long id, SessionUserDto sessionUserDto) {
        Comment comment = commentRepository.findById(id)
                .orElseThrow(() -> new CommentNotFoundException("해당 Comment를 찾을 수 없습니다."));
